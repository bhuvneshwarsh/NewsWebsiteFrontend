...
import { Plus, Pencil, UserX, UserCheck, Trash2, Upload, X, User, QrCode } from 'lucide-react';
...
function EmployeeFormModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = existing
        ? await api.put<{ success: boolean; data: EmployeeAdmin }>(`/employees/${existing.id}`, form)
        : await api.post<{ success: boolean; data: EmployeeAdmin }>('/employees', form);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto"
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-5">
            <div className="shrink-0">
              {form.imageUrl && !imgErr ? (
                ...
                <div className="w-20 h-20 rounded-full bg-gray-100 border-2 border-dashed
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Full Name *"    value={form.fullName}       onChange={v => set('fullName', v)}       placeholder="e.g. Rajesh Kumar" />
            <Field label="Designation *"  value={form.designation}    onChange={v => set('designation', v)}    placeholder="e.g. Senior Reporter" />
            <Field label="Email"           value={form.email ?? ''}   onChange={v => set('email', v)}          placeholder="employee@email.com" type="email" />
            ...
            <Field label="Display Order"    value={String(form.displayOrder ?? 0)} onChange={v => set('displayOrder', parseInt(v) || 0)} type="number" placeholder="0 = first" />
    ...
    <div className="p-6">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-5
        ...
        <p className="text-xs text-blue-700">
          Click the <strong>QR</strong> button on any employee to generate their unique QR code
          for printing on physical ID cards. Scanning the QR opens their verified digital profile.
        </p>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                <tr>
                  {['Photo','Name','Employee ID','Designation','Valid Upto','Status','Actions'].map(h => (
                ...
                {employees.map(emp => (
                  <tr key={emp.id}
                    ...
                    <td className="px-4 py-3 text-xs whitespace-nowrap">
                      {emp.validUpto ? (
                        <span className={new Date(emp.validUpto) < new Date()
                          ? 'text-red-500 font-medium' : 'text-green-600'}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setQrFor(emp)}
                          className="flex items-center gap-1 bg-brand-50 hover:bg-brand-100
                            text-brand-600 border border-brand-200 px-2 py-1 rounded-lg
                            text-xs font-medium transition" title="Generate QR Code">
                          <QrCode size={13} /> QR
                        </button>
      ...
      {qrFor && (
        <QRCodeModal
          employeeId={qrFor.employeeId}
          fullName={qrFor.fullName}
